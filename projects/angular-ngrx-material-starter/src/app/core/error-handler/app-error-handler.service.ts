...
export class AppErrorHandler extends ErrorHandler {
  ...
  handleError(error: Error | HttpErrorResponse) {
    ...
    if (error && error instanceof  HttpErrorResponse && error.error &&  error.error.message && error.error.message.indexOf('token has expired') >= 0){
      this.notificationsService.errorLong('Sesiunea a expirat!. Autentifica-te din nou');
      const store = this.injector.get(Store);
      if (store){
        store.dispatch(authLogout({redirect: false}));
      }
    } else if (error && error instanceof  HttpErrorResponse && error.error &&  error.error.message && error.error.message.indexOf('Unauthorized') >= 0){
      this.notificationsService.errorLong('Acces respins. Autentifica-te sau cere drepturi');
    } else if (error && error instanceof  HttpErrorResponse && error.status === 403 && error.url && error.url.indexOf('https://tableta-inmultirii-public.s3.eu-central-1.amazonaws.com') < 0){
